    ...
    pages: [
      {
        ...
        apis: [
          { route: '/api/ft/users', tables: ['ft_users'] },
          { route: '/api/get-customer-balance', tables: ['invoiceManager_transactions', 'invoiceManager_balance', 'ft_users', 'ft_cancel_details'] },
          { route: '/api/get-payment-transactions', tables: ['invoiceManager_transactions'] },
        ],
        note: '구 원장 조회 전용 (2026-09-14 부터 기록 기능 제거). 충전·차감·적용일 수정은 고객계좌 (신)',
      },
      {
        name: '고객계좌 (신)',
        route: '/invoice/payment-history-v2',
        file: 'app/invoice/payment-history-v2/PaymentHistoryV2.tsx',
        apis: [
          { route: '/api/ft/users', tables: ['ft_users'] },
          { route: '/api/ft/user-transactions', tables: ['ft_user_transactions', 'ft_users', 'ft_balances'] },
          { route: '/api/ft/user-transactions/order-deduct', tables: ['ft_user_transactions', 'ft_users', 'ft_balances', 'ft_orders'] },
          { route: '/api/exchange-rate', tables: [] },
        ],
        note: '신 원장(ft_user_transactions)이 유일한 기준. 충전·차감·1688 주문 차감·적용일 수정은 DB 함수(record_manual_transaction_v2 / deduct_balance_and_record_transaction_v2)로 기록. 구 원장은 읽지 않음',
      },
      {
        name: '무역계좌',
        ...
        note: '고객계좌(PaymentHistory)를 그대로 재사용 — 구 원장 조회 전용',
      },
    ],
  },
