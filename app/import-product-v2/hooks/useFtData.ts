...
//   - 비고검색: note_cn 단독 검색 (다른 배송번호가 메모된 경우 등)
// ============================================================
export function useFtSearch(items: FtOrderItem[], searchType: string = '일반검색') {
  ...
  const filteredItems = useMemo(() => {
    ...
    // 비고검색 모드: note_cn 로만 필터
    if (searchType === '비고검색') {
      return items.filter((item) =>
        (item.note_cn || '').toLowerCase().includes(term)
      );
    }
...
// ============================================================
// useFtFulfillmentSummary — ARRIVAL / CANCEL / RETURN / PACKED / SHIPMENT 합산
//
// items가 변경될 때마다 ft_fulfillments를 일괄 조회 (1회 fetch)
// type별로 order_item_id → quantity 합계 Map을 각각 반환
//
// arrivalMap     : type=ARRIVAL                → 입고 열
// cancelMap      : type=CANCEL                 → 취소 (주문 취소)
// returnMap      : type=RETURN                 → 반품 (입고 후 돌려보냄)
// packedMap      : type=PACKED                 → 포장 (전체)
// shipmentMap    : type=SHIPMENT               → 출고
// exportMap      : PACKED+shipment_id IS NOT NULL (product_id 기준) — 출고완료 표시용
// shippedItemMap : PACKED+shipment_id IS NOT NULL (order_item_id 기준) — 진행 공식용
// ============================================================
    ...
    const fetchSummary = async () => {
      ...
      try {
        ...
        if (json.success) {
          ...
          const ret      = new Map<string, number>();   // RETURN
          const shipment = new Map<string, number>();
          const exportMap = new Map<string, number>();      // PACKED + shipment_id NOT NULL → product_id 기준
          const shippedItem = new Map<string, number>();    // PACKED + shipment_id NOT NULL → order_item_id 기준 (진행 공식용)
